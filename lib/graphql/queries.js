/**
 * GraphQL queries for WordPress
 *
 * These queries use WPGraphQL schema.
 * Make sure WPGraphQL plugin is installed and activated on your WordPress site.
 */

import { gql } from "@apollo/client";

/**
 * Fragment for post fields - reusable across queries
 */
const POST_FIELDS = gql`
  fragment PostFields on Post {
    id
    title
    slug
    date
    modified
    excerpt
    content
    status
    commentCount
    featuredImage {
      node {
        sourceUrl
        altText
        mediaDetails {
          width
          height
        }
      }
    }
    topics {
      nodes {
        id
        name
        slug
      }
    }
    postYears {
      nodes {
        id
        name
        slug
      }
    }
    geographies {
      nodes {
        id
        name
        slug
      }
    }
    news {
      gallery {
        nodes {
          sourceUrl
          altText
          mediaDetails {
            width
            height
          }
        }
      }
      image {
        node {
          sourceUrl
          altText
          mediaDetails {
            width
            height
          }
        }
      }
    }
  }
`;

/**
 * Get posts query
 */
export const GET_POSTS = gql`
  ${POST_FIELDS}
  query GetPosts(
    $first: Int
    $after: String
    $where: RootQueryToPostConnectionWhereArgs
  ) {
    posts(first: $first, after: $after, where: $where) {
      edges {
        node {
          ...PostFields
        }
        cursor
      }
      pageInfo {
        hasNextPage
        hasPreviousPage
        startCursor
        endCursor
      }
    }
  }
`;

/**
 * Taxonomy terms for news post filters
 */
export const GET_POST_FILTERS = gql`
  query GetPostFilters {
    topics(first: 100) {
      nodes {
        id
        name
        slug
        count
      }
    }
    postYears(first: 100) {
      nodes {
        id
        name
        slug
        count
      }
    }
    geographies(first: 100) {
      nodes {
        id
        name
        slug
        count
      }
    }
  }
`;

/**
 * Get single post by slug
 */
export const GET_POST_BY_SLUG = gql`
  ${POST_FIELDS}
  query GetPostBySlug($slug: ID!) {
    post(id: $slug, idType: SLUG) {
      ...PostFields
      author {
        node {
          id
          name
          slug
        }
      }
    }
  }
`;

/**
 * Get single post by ID
 */
export const GET_POST_BY_ID = gql`
  ${POST_FIELDS}
  query GetPostById($id: ID!) {
    post(id: $id, idType: DATABASE_ID) {
      ...PostFields
    }
  }
`;

/**
 * Get pages query
 */
export const GET_PAGES = gql`
  fragment PageFields on Page {
    id
    title
    slug
    date
    modified
    excerpt
    content
    author {
      node {
        id
        name
        slug
      }
    }
    featuredImage {
      node {
        sourceUrl
        altText
        mediaDetails {
          width
          height
        }
      }
    }
  }

  query GetPages($first: Int, $after: String) {
    pages(first: $first, after: $after) {
      edges {
        node {
          ...PageFields
        }
        cursor
      }
      pageInfo {
        hasNextPage
        hasPreviousPage
        startCursor
        endCursor
      }
    }
  }
`;

/**
 * Get single page by slug
 */
export const GET_PAGE_BY_SLUG = gql`
  fragment PageFields on Page {
    id
    title
    slug
    date
    modified
    excerpt
    content
    author {
      node {
        id
        name
        slug
      }
    }
    featuredImage {
      node {
        sourceUrl
        altText
        mediaDetails {
          width
          height
        }
      }
    }
  }

  query GetPageBySlug($slug: ID!) {
    page(id: $slug, idType: URI) {
      ...PageFields
    }
  }
`;

/**
 * Get categories
 */
export const GET_CATEGORIES = gql`
  query GetCategories($first: Int) {
    categories(first: $first) {
      nodes {
        id
        name
        slug
        description
        count
      }
    }
  }
`;

/**
 * Get posts by category
 */
export const GET_POSTS_BY_CATEGORY = gql`
  ${POST_FIELDS}
  query GetPostsByCategory($categoryId: ID!, $first: Int) {
    posts(first: $first, where: { categoryId: $categoryId }) {
      edges {
        node {
          ...PostFields
        }
      }
    }
  }
`;

/**
 * Search posts
 */
export const SEARCH_POSTS = gql`
  ${POST_FIELDS}
  query SearchPosts($search: String!, $first: Int) {
    posts(first: $first, where: { search: $search }) {
      edges {
        node {
          ...PostFields
        }
      }
    }
  }
`;

/**
 * Fragment for publication fields - reusable across queries
 */
const RESOURCE_FIELDS = gql`
  fragment ResourceFields on Resource {
    id
    title
    slug
    date
    modified
    excerpt
    content
    featuredImage {
      node {
        sourceUrl
        altText
        mediaDetails {
          width
          height
        }
      }
    }
    textInputs {
      authors
    }
  }
`;

/**
 * Get publications query
 */
export const GET_RESOURCES = gql`
  query GetResources(
    $first: Int
    $after: String
    $where: RootQueryToResourceConnectionWhereArgs
  ) {
    resources(first: $first, after: $after, where: $where) {
      edges {
        node {
          id
          title
          slug
          date
          modified
          excerpt
          content
          featuredImage {
            node {
              sourceUrl
              altText
              mediaDetails {
                width
                height
              }
            }
          }
          textInputs {
            authors
          }
          formats {
            nodes {
              id
              name
              slug
            }
          }
          geographies {
            nodes {
              id
              name
              slug
            }
          }
          thematicAreas {
            nodes {
              id
              name
              slug
            }
          }
          resourcesTypes {
            nodes {
              id
              name
              slug
            }
          }
        }
        cursor
      }
      pageInfo {
        hasNextPage
        hasPreviousPage
        startCursor
        endCursor
      }
    }
  }
`;

export const GET_RESOURCE_FILTERS = gql`
  query GetResourceFilters {
    formats(first: 100) {
      nodes {
        id
        name
        slug
        count
      }
    }
    geographies(first: 100) {
      nodes {
        id
        name
        slug
        count
      }
    }
    thematicAreas(first: 100) {
      nodes {
        id
        name
        slug
        count
      }
    }
    resourcesTypes(first: 100) {
      nodes {
        id
        name
        slug
        count
      }
    }
  }
`;

const RESOURCE_LANGUAGE_FILES = gql`
  fragment ResourceLanguageFiles on Resources {
    alb {
      node {
        uri
        sourceUrl
        mediaItemUrl
      }
    }
    english {
      node {
        uri
        sourceUrl
        mediaItemUrl
      }
    }
    bosniaAndHerzegovina {
      node {
        uri
        sourceUrl
        mediaItemUrl
      }
    }
    croatia {
      node {
        uri
        sourceUrl
        mediaItemUrl
      }
    }
    kosovo {
      node {
        uri
        sourceUrl
        mediaItemUrl
      }
    }
    montenegro {
      node {
        uri
        sourceUrl
        mediaItemUrl
      }
    }
    northMacedonia {
      node {
        uri
        sourceUrl
        mediaItemUrl
      }
    }
    serbia {
      node {
        uri
        sourceUrl
        mediaItemUrl
      }
    }
    slovenia {
      node {
        uri
        sourceUrl
        mediaItemUrl
      }
    }
    turkie {
      node {
        uri
        sourceUrl
        mediaItemUrl
      }
    }
  }
`;

/**
 * Get single publication by slug
 */
export const GET_RESOURCE_BY_SLUG = gql`
  ${RESOURCE_LANGUAGE_FILES}
  query GetResourceBySlug($slug: ID!) {
    resource(id: $slug, idType: SLUG) {
      id
      title
      slug
      date
      modified
      excerpt
      content
      featuredImage {
        node {
          sourceUrl
          altText
          mediaDetails {
            width
            height
          }
        }
      }
      textInputs {
        authors
      }
      resources {
        ...ResourceLanguageFiles
      }
    }
  }
`;
